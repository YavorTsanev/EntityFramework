...
namespace MiniORM
{
    ...
    public abstract class DbContext
    {
            ...
            where TCollection : class, new()
        {
            ...
            var collectioType = typeof(TCollection);

            var primaryKeys = collectioType.GetProperties()
                .Where(pi => pi.HasAttribute<KeyAttribute>())
                .ToArray();

            var primayKey = primaryKeys.First();
            ...
            if (isManyToMany)
            {
                primayKey = collectioType.GetProperties()
                    .First(pi => collectioType
                    .GetProperty(pi.GetCustomAttribute<ForeignKeyAttribute>().Name)
                    .PropertyType == entityType);
            }

            var navigationDbSet = (DbSet<TCollection>)this.dbSetProperties[collectioType].GetValue(this);
            ...
            foreach (var entity in dbSet)
            {
                ...
                var navigationEntities = navigationDbSet
                    .Where(navigationEntity => primayKey.GetValue(navigationEntity).Equals(primaryKeyValue))
                    .ToArray();
        ...
        private void MapNavigatonProperties<TEntity>(DbSet<TEntity> dbSet)
            where TEntity : class, new()
        {
            var entityType = typeof(TEntity);
